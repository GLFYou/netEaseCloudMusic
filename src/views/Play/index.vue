<template>
  <div>
    ...
    <div class="toggle" :style="`transform: translateX(-50%)  rotate(${arrowDeg})`" @click="toggle"><van-icon name="arrow-up" /><br /><van-icon name="arrow-down" /></div>
  </div>
</template>

<script>
// 获取歌曲详情和 歌曲的歌词接口
import { getSongByIdAPI, getLyricByIdAPI } from '@/api/index.js'
...
export default {
  ...
  data() {
    return {
      playState: true, // 音乐播放状态(true暂停, false播放)
